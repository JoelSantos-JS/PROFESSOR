...
### 9.1. Modo "Professor substituto" — conversação com IA (treino de speaking)

Um **chat** onde a IA age como um **professor que conversa com o aluno**, usando como
**contexto a transcrição daquela sessão do Tutor Board** (as frases que apareceram no
conteúdo assistido). A ideia central:

- **Contexto = a sessão atual.** A IA pega a transcrição/frases daquela sessão e usa **só
  isso** como base do papo — fica "dentro do assunto" do que o usuário acabou de assistir.
- **A IA só PERGUNTA e conversa.** Ela age como um professor substituto que **faz perguntas**
  sobre o contexto e puxa a conversa, em vez de só explicar. O aluno **responde** (idealmente
  falando / praticando speaking).
- **Feedback de professor.** Depois que o aluno responde, a IA:
  - dá **formas melhores de responder** (reformula a resposta do aluno de um jeito mais
    natural/correto);
  - mostra **como o aluno deveria responder numa situação similar** (modelos de resposta);
  - corrige e sugere, mantendo o tom de professor que ensina pela conversa.
- **Resumo:** é um **professor substituto que só pergunta e conversa** de acordo com o
  contexto da sessão do Tutor Board — transformando a transcrição passiva em uma **conversa
  guiada de prática**, com correção e exemplos de como responder.

**Fluxo esperado:**
1. Usuário assiste algo → frases vão para o Tutor Board (contexto da sessão).
2. Abre o modo conversa → a IA puxa o contexto da sessão e **faz uma pergunta** relacionada.
3. Usuário **responde** (fala/escreve).
4. IA dá feedback: melhor forma de dizer + como responderia numa situação parecida → e faz
   a **próxima pergunta**, mantendo a conversa fluindo.

**Implicações para o design:** uma nova tela/painel de **chat** (bolhas de conversa), com
distinção clara entre "pergunta da IA", "resposta do usuário" e "feedback/correção". Deve
deixar evidente que a IA está **conversando**, não dando aula. Provável ponto de entrada:
um botão no Tutor Board ("Conversar sobre esta sessão"). Reaproveita o **trio de áudio** e o
**rigor no speaking** (a resposta do usuário pode ser falada e avaliada).
